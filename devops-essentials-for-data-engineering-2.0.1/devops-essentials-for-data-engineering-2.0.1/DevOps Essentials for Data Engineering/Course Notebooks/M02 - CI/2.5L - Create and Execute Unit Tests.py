...
# MAGIC
# MAGIC
# MAGIC def convert_milles_to_km(df, new_column_name, miles_column):
...
# MAGIC
# MAGIC     **NOTE:** There are a variety of ways to test this function to. We will keep it simple for this lab.
# MAGIC
...
# MAGIC %md
# MAGIC 2. Complete the unit test `test_convert_miles_to_km_function` to test the custom `transforms.convert_milles_to_km` function. Use the `pyspark.testing.utils.assertDataFrameEqual` function to test the actual DataFrame against the expected DataFrame.
# MAGIC
...
# MAGIC
# MAGIC     **NOTE:** If you are completing the challenge, modify the path to to test your specific **.py** file.
